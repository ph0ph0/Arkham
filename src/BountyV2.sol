...
contract BountyV2 is Ownable {
    ...
    uint256 private _submissionStake; // How much a submitter must stake in ARKM to make a submission
    uint256 private _makerFee; // How much the bounty creater is charged for creating the bounty
    uint256 private _takerFee; // @todo - How much the submitter is charged for completing the bounty. Goes to protocol?
    uint32 private _maxActiveSubmissions; // @todo - How many active submissions are allowed at a time?
    bool private _acceptingBounties; // @todo - Does this prevent new bounties from being created? Or from submitting?
    // @target-high: Slither - cross-function reentrancy
    ...
    event FundBounty(uint256 indexed bountyID, address indexed funder, uint256 initialValue, uint64 expiration);
    ...
    event SetSubmissionStake(uint256 newStake, uint256 oldStake);
    ...
    // @follow-up: add check that the token is ARKM
    constructor(
        address arkmAddress,
        uint256 initialSubmissionStake,
        uint256 initialMakerFee,
        uint256 initialTakerFee,
        uint32 bountyDuration,
        address feeReceiverAddress,
        uint32 initialMaxActiveSubmissions,
        uint256 initialMinBounty
    ) {
    ...
    function submissionQueuePosition(uint256 submission, uint256 bounty) public view virtual returns (uint32) {
        ...
        for (uint32 i = 0; i < _numSubmissions; i++) {
            if (
                _submissions[bounty][i].payload
                    == keccak256(abi.encodePacked(submission, _submissions[bounty][i].submitter))
            ) {
                return i;
            }
        }
    ...
    function fundBounty(uint256 bounty, uint256 _amount) external {
        ...
        emit FundBounty(bounty, _msgSender(), _amount, _expiration);
    }
    ...
    function closeBounty(uint256 bounty) external {
        ...
        require(
            expiration(bounty) <= block.timestamp || approver(_msgSender()),
            "BountyV2: only approvers can close before expiration"
        );
    ...
    function makeSubmission(uint256 bounty, bytes32 payload) external {
        ...
        _submissions[bounty].push(Submission({payload: payload, submitter: _msgSender(), stake: _submissionStake}));
    ...
    // @target-high: Slither - Possible Re-Entrancy
    ...
    function setSubmissionStake(uint256 newStake) external onlyOwner {
        ...
        emit SetSubmissionStake(newStake, _oldStake);
    }
